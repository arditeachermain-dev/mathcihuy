...
-- 4.2 Kebijakan Tabel SISWA
-- Siswa & Publik (anon) hanya boleh membaca data siswa untuk verifikasi (SELECT)
...
-- Hanya Guru terautentikasi (authenticated) yang boleh mengelola master siswa (ALL)
...
-- 4.3 Kebijakan Tabel NILAI_CBT
-- Siswa & Publik dapat melihat hasil nilai CBT (SELECT)
...
-- - Skor harus dalam rentang 0 s.d. 100
-- - Jumlah soal > 0 dan jumlah benar <= jumlah soal
-- - Total (benar + salah) <= jumlah soal
-- - Anon TIDAK memiliki izin UPDATE (mencegah manipulasi skor jadi 100 via DevTools/Postman)
-- - Anon TIDAK memiliki izin DELETE (mencegah penghapusan database nilai kawan-kawan)
...
-- Guru dapat mereset nilai ujian (DELETE) atau mengoreksi nilai (UPDATE)
...
-- 4.4 Kebijakan Tabel CBT_LIVE_ANSWERS
...
-- Siswa (anon) TIDAK DIBERI IZIN DELETE pada cbt_live_answers (mencegah sabotase live answers)
-- Hanya Guru terautentikasi yang dapat menghapus / mereset live answers (ALL)
