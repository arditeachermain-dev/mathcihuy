...
function catatSesiCbt(subj, pkgId, forceSubmit) {
    ...
    function confirmResetCbtPackage() {
      ...
      const subjNames = { wajib: 'Matematika Wajib', minat: 'Matematika Peminatan', clil: 'CLIL' };
      const namaMapel = subjNames[curSubj] || curSubj;

      const pesan = `Apakah Anda yakin ingin mengulang pengerjaan paket ${curPkg} (${namaMapel})?\n\n` +
                    `⚠️ PERHATIAN:\n` +
                    `• Hanya jawaban pada paket ${curPkg} ini yang akan dikosongkan kembali dari Soal 1.\n` +
                    `• Jawaban paket bab/pertemuan lain TETAP AMAN dan TIDAK akan terhapus.\n\n` +
                    `Klik OK untuk mengulang paket ${curPkg}, atau Batal untuk kembali.`;
    ...
    function retakeCurrentTkaPkg() {
      ...
      // 1. Bersihkan HANYA skor sesi untuk paket ini dari memori objek userSessionScores
      if (typeof userSessionScores === 'object' && userSessionScores !== null) {
        Object.keys(userSessionScores).forEach(k => {
          if (k.startsWith(`${curSubj}_${curPkg}_`) || k.includes(`_${curPkg}_`)) {
            delete userSessionScores[k];
          }
        });
      }

      // 2. Bersihkan draf jawaban HANYA untuk paket ini secara terisolasi
      ...
      // 6. Tampilkan notifikasi visual toast informatif
      const toast = document.createElement('div');
      toast.className = 'fixed bottom-6 right-6 z-50 px-4 py-2.5 rounded-2xl bg-[#0D1B2E] border border-blue-500/80 text-white text-xs font-bold shadow-2xl flex items-center gap-2 animate-bounce';
      toast.innerHTML = `<i class="fa-solid fa-rotate-left text-amber-400"></i> <span>Paket ${curPkg} berhasil diulang dari Soal 1. Paket lain tetap aman!</span>`;
      document.body.appendChild(toast);
      setTimeout(() => toast.remove(), 3500);
...
function showTkaScorecardModal() {
    ...
    function tandaiOpsiTunggal(chosen, correct) {
      ...
      document.querySelectorAll('.opt-btn').forEach(b => {
        ...
        const span = mSpan(b);
        const gaya = (L === K) ? (L === C ? OPT_RIGHT : OPT_MISSED)
                   : (L === C) ? OPT_WRONG : OPT_MUTED;
        b.disabled = true;
